...
var getDatacrawler = require('../crawler/getDatacrawler');

myEvents.on('gameover', function () {
    // getdata.uploadServe('ingkee');
});
var isRunning = false;
exports.Oriingkee0801 = function () {
    if (isRunning) {
        return false;
    } else {
        isRunning = true;
...
myEvents.on('start', function () {
    ...
    schedule.scheduleJob(rule, function () {
        if (getDatacrawler.updateFans()) {
            this.cancel();
            console.log('update orignal_ingkee_2016_08_01 score start');
